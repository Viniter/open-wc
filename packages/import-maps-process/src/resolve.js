...
const supportedBuiltInModules = new Set(['std:blank']);
  ...
  for (const [specifierKey, addresses] of Object.entries(specifierMap)) {
    ...
    if (specifierKey === normalizedSpecifier) {
      if (addresses.length === 0) {
        ...
        if (singleAddress.protocol === 'std' && !supportedBuiltInModules.has(singleAddress.href)) {
          throw new TypeError(`The "${singleAddress.href}" built-in module is not implemented.`);
        }
        return singleAddress;
      } else if (
        addresses.length === 2 &&
        addresses[0].protocol === 'std' &&
        addresses[1].protocol !== 'std'
      ) {
...
export function resolve(specifier, parsedImportMap, scriptURL) {
  const asURL = parseUrl(specifier, scriptURL);
  ...
  if (asURL) {
    if (asURL.protocol === 'std' && !supportedBuiltInModules.has(asURL.href)) {
